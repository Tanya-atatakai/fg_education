...
from .utils.exceptions import NotInMutableTagsException
from .utils.parsers import JsonParserSample_1
...
class Runner:
    ...
    def convert_json2html(cls, input_data):
        ...
        Иначе входные данные парсятся в удобный для
        перебора тип, а затем идет конвертация в html с заменой атрибутов title
        и body на h1 и p соответственно.

        :param input_data: данные, которые будут обрабатываться парсером
        :type input_data: str
        :return: ошибку валидации либо ошибку неверных тегов,
        либо сконвертированные данные в result
        ...
        parsed_input = JsonParserSample_1()

        try:
            result = parsed_input.json_list_to_dict(loads_input)
        except NotInMutableTagsException as error:
